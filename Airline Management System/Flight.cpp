...
void Flight::printSummary(){
    ...
    for (auto ticket : bookedTickets){
        if(ticket->getBookingStatus() == BookingStatus::B){
    ...
    for (auto ticket : bookedTickets){
        ...
        if (ticket->getMealRequired() == 'Y' && ticket->getBookingStatus() == BookingStatus::B){
            auto seatNo = ticket->getSeatNos();
            for (auto num : seatNo){
                cout << num << ", ";
            }
